...
export default function BankSoalPage() {
  ...
  // Soal form — SEPARATE state, not aliased
  ...
  const loadBanks = async () => {
    const [{ data: banks }, { data: soal }, { data: bsm }, { data: bsk },
           { data: mapels }, { data: kelass }] = await Promise.all([
  ...
  // ── Soal CRUD — explicit state, no aliasing ───────────────────
  const openSoalForm = (q?: Question) => {
    if (q) {
      setSFId(q.id); setSFNum(q.number.toString())
      setSFQ(q.question); setSFActive(q.is_active)
    } else {
      setSFId(''); setSFNum(''); setSFQ(''); setSFActive(true)
    }
    // set view AFTER state so soal-form renders fresh values
    setView('soal-form')
  }
  ...
  const saveSoal = async () => {
    ...
    const payload = {
      ...
      mata_pelajaran_id: bankMapelIds[0] || null,
    }
    if (soalFormId) {
      await supabase.from('questions').update(payload).eq('id', soalFormId)
    } else {
      await supabase.from('questions').insert(payload)
    }
    // Refresh soal list
    const { data: fresh } = await supabase.from('questions')
      .select('*').eq('bank_soal_id', selBank.id).order('number')
    setSoalList(fresh || [])
    await loadBanks()
    setSoalSaving(false)
    flash('✅ Soal disimpan!')
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async evt => {
      ...
      setSoalList(fresh || [])
      await loadBanks()
      setImp(false); flash(`✅ ${rows.length} soal diimport!`)
      e.target.value = ''
    }
  ...
  const downloadTemplate = () => {
    ...
    a.download = `template-soal-${selBank?.nama.replace(/\s+/g,'-')||'bank'}.csv`
    a.click()
  }

  // ── Penilaian ─────────────────────────────────────────────────
  const openJadwalList = async () => {
    if (!selBank || bankMapelIds.length === 0) {
      flash('⚠️ Bank soal belum punya mata pelajaran. Assign mapel dulu.'); return
    }
    const { data } = await supabase
      .from('jadwal_ujian')
      .select('id,nama,tanggal,mata_pelajaran_id,mata_pelajaran:mata_pelajaran_id(nama),kelas:kelas_id(nama)')
      .in('mata_pelajaran_id', bankMapelIds)
  ...
  const Header = ({ title, sub, back }: { title: string; sub?: string; back?: () => void }) => (
    <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-40">
      <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
        <button onClick={back ?? (() => router.push('/admin'))}
          className="p-1.5 text-slate-400 hover:text-slate-200 rounded-lg hover:bg-slate-800 flex-shrink-0">
  ...
  if (view === 'penilaian-detail' && selSession) {
    ...
    return (
      <div className="min-h-screen bg-slate-950">
        <Header title={selSession.full_name}
          sub={`${selSession.npm} · ${selSession.username} · ${selJadwal?.nama}`}
          back={() => setView('penilaian')} />
        <div className="max-w-3xl mx-auto p-4 space-y-4">
          ...
          <div className="card p-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => dlSiswa(selSession)} className="btn-secondary text-xs py-1.5 flex items-center gap-1">
                <Ic d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>DL
              </button>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-800">
                ...
                <div>
                  ...
                  <input type="text"
                    ...
                    onBlur={() => saveScore(a.id)}
                    placeholder="Catatan..." className="input-field text-sm"/>
                </div>
              </div>
  ...
  if (view === 'penilaian') return (
    <div className="min-h-screen bg-slate-950">
      <Header title={`Penilaian — ${selJadwal?.nama}`}
        sub={`${selJadwal?.mapel_nama} · ${selJadwal?.kelas_nama||'Semua Kelas'}`}
        back={() => setView('jadwal-list')} />
      <div className="max-w-5xl mx-auto p-4">
        ...
        <div className="card p-4 mb-4 flex flex-wrap items-center justify-between gap-3">
          <input type="text" value={gradSearch} onChange={e => setGS(e.target.value)}
            placeholder="Cari nama / NPM / username..." className="input-field text-sm py-2 flex-1 min-w-[200px] max-w-xs"/>
  ...
  if (view === 'jadwal-list') return (
    <div className="min-h-screen bg-slate-950">
      <Header title={`Penilaian — ${selBank?.nama}`} sub="Pilih jadwal ujian"
        back={() => setView('bank-detail')} />
  ...
  if (view === 'bank-detail' && selBank) return (
    <div className="min-h-screen bg-slate-950">
      <Header title={selBank.nama}
        sub={`${soalList.length} soal · ${bankMapelIds.length} mapel · ${bankKelasIds.length} kelas`}
        back={() => { setSelBank(null); setView('bank-list') }} />
      <div className="max-w-5xl mx-auto p-4">
        <Flash/>
          ...
          <div>
            <div className="flex flex-wrap gap-2 mb-4">
              <button onClick={() => openSoalForm()} className="btn-primary text-sm py-2 flex items-center gap-1.5">
                <Ic d="M12 4v16m8-8H4"/>Tambah Soal
              </button>
              <button onClick={openJadwalList} className="btn-secondary text-sm py-2 flex items-center gap-1.5">
                <Ic d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>Penilaian
              </button>
              <button onClick={downloadTemplate} className="btn-secondary text-sm py-2 flex items-center gap-1.5">
                <Ic d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>Template CSV
              </button>
                  ...
                  {soalList.map(q => (
                    <div key={q.id} className={`card p-4 flex items-start gap-3 ${!q.is_active?'opacity-50':''}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className={`text-xs mt-1 inline-block px-2 py-0.5 rounded ${q.is_active?'text-green-400 bg-green-500/10':'text-slate-500 bg-slate-800'}`}>
              ...
              <div className="space-y-0.5 max-h-52 overflow-y-auto">
                {mapelAll.map(m => (
                  <label key={m.id} className="flex items-center gap-2.5 p-2 rounded-lg hover:bg-slate-800 cursor-pointer">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm text-slate-200 truncate">{m.nama}</p>
                      <p className="text-xs text-slate-500">{m.kode}</p>
                    </div>
                  </label>
                ))}
      ...
      <div className="max-w-5xl mx-auto p-4">
        ...
        <div className="card p-5 mb-5">
          <h2 className="text-sm font-semibold text-slate-100 mb-3">
            {editBankId ? 'Edit Bank Soal' : 'Buat Bank Soal Baru'}
          </h2>
          <div className="flex flex-wrap gap-3">
            <input type="text" value={bankForm.nama}
              onChange={e => setBF(p => ({ ...p, nama: e.target.value }))}
              placeholder="Nama bank soal (cth: UTS Ganjil 2025 — Kelas A)" className="input-field flex-1 min-w-[220px] text-sm py-2"/>
            <input type="text" value={bankForm.deskripsi}
              onChange={e => setBF(p => ({ ...p, deskripsi: e.target.value }))}
              placeholder="Deskripsi (opsional)" className="input-field flex-1 min-w-[160px] text-sm py-2"/>
            <div className="flex gap-2">
              <button onClick={saveBank} disabled={!bankForm.nama.trim()}
                className="btn-primary text-sm py-2 whitespace-nowrap">
                {editBankId ? 'Simpan Perubahan' : '+ Buat Bank Soal'}
              </button>
              {editBankId && (
                <button onClick={() => { setBF({ nama:'',deskripsi:'',is_active:true }); setEBI(null) }}
                  className="btn-secondary text-sm py-2">Batal</button>
              )}
            </div>
          </div>
                  ...
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2 flex-shrink-0">
                      ...
                      <button onClick={() => deleteBank(b.id)}
                        className="text-xs text-red-500 hover:text-red-400 px-1.5 py-1 rounded hover:bg-slate-800">Hapus</button>
                    </div>
                  </div>
